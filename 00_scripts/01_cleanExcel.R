# Clean excel data
# "00_scripts/01_cleanExcel.R"

# This script will:
#   - read in excel data files that are in the 01_input folder. Each subfolder in "01_input" will be cleaned as one output.
...
#Input files
#   - Campbell xls files should have two columns:
#       "DateTime" with a format of "%Y-%m-%d %H:%M" and "mm"
#   - HoBo xls files should have four columns:
#       "DateTime" with a format of "%Y-%m-%d %H:%M:%S", "Temperature_C", "Event"and "mm"

# Version: 0.2
# Last update: 2023-03-24
# SC update: 2023-04-03
#     - Section 1.2.1 and Section 1.2.2.
#         changed file read-in format and process to work with xls files instead of the logger dat files
#     - Section 2.4
...
for (location_dir_i in location_dir) {                                          # e.g.   location_dir_i  <- "01_input/Burn"
  ...
  for (data_type_dir_i in data_type_dir) {                                      # data_type_dir_i  <-  "01_input/Burn/Campbell"
    ...
    if (str_detect(data_type_nicename, regex("Campbell", ignore_case = TRUE))) {
      data_mytest <- map_dfr(.x = filelist, .f = read_excel)%>%
        rename(Rain_mm = mm)%>%
